...
```
# appjail-dns
10.42.0.1       development.appjail
10.42.0.2       golang.development.appjail
10.42.0.3       hello.development.appjail
10.42.0.4       python.development.appjail
172.0.0.1       testing.appjail
```

As you can see, it generates a list similar to `hosts(5)`. This list can be used with a DNS server such as `dns/dnsmasq`.
...
Our `dnsmasq.conf` will add an additional `hosts(5)` file using the `addn-hosts` option which expects a file or a directory. In this case, a directory.

```
# mkdir -p /etc/hosts.d
# touch /etc/hosts.d/appjail
```

The other options like `no-resolv` to not load `/etc/resolv.conf` and `server` to specify a name server. We need to modify `/etc/resolv.conf`, however we cannot simply edit this file as `resolvconf(5)` will modify it losing all the changes we have made.

**/etc/resolvconf.conf**:

```
name_servers="192.168.1.106"
```
...
`192.168.1.106` is the host IPv4 address. Be careful to use an IPv4 address that a jail can't reach as the jail is likely to copy the host's resolv.conf.

Now we can run `dnsmasq`:
...
Now that we have our DNS configured and have made changes to our DNS configuration, we can proceed generating the host list.

**/usr/local/bin/scripts/ajdns.sh**:
...
AJHOSTS="/etc/hosts.d/appjail"

HOOK="/usr/local/bin/scripts/ajdns.hook.sh"

HOSTS=`appjail-dns`
HOSTS_SUM=`printf "%s\n" "${HOSTS}" | ${ALGO}`

if [ `${ALGO} -q "${AJHOSTS}"` != ${HOSTS_SUM} ]; then
        printf "%s\n" "${HOSTS}" > "${AJHOSTS}"
        "${HOOK}" "${AJHOSTS}"
fi
```

The idea with the above script is to use `appjail-dns` to get the list of hosts and check if differs with the current list. If the list differs, it will be updated to proceed to execute the hook.

**/usr/local/bin/scripts/ajdns.hook.sh**:
...
if [ -z "${HOSTS}" ]; then
        echo "usage: ajdns.hook.dns hosts" >&2
        exit 64 # EX_USAGE
fi
...
The idea with the hook is to parse the hosts list file to a specific format that our DNS server can understand. In this case, such a conversion is not necessary since `dnsmasq` expects a `hosts(5)` file that our file is already compatible with, so we only have to reload our DNS server.

All we have to do is run the script.

```sh
# /usr/local/bin/scripts/ajdns.sh
# cat /etc/hosts.d/appjail
10.42.0.1       development.appjail
10.42.0.2       golang.development.appjail
10.42.0.3       hello.development.appjail
10.42.0.4       python.development.appjail
...
We need to run at intervals to update the host list, so `cron(8)` is our best friend.
...
# AppJail DNS
*/3     *       *       *       *       root    /usr/local/bin/scripts/ajdns.sh
```

To check if all changes have been correctly, just send an ICMP packet to a jail with its status in UP.
...
### Maintenance of installed Images
