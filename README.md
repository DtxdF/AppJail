...
See `Source tree` for more information on updating a release and a jail.
...
Although AppJail does not include a DNS server, it includes some useful scripts for using your favorite DNS server. We just need to enable and start `appjail-dns` and our DNS server. AppJail comes with a configuration file for dnsmasq, so we will use it for these examples.
...
Tip: Instead of using the host IP we can use `if_tap(4)`. This is because if our IP address is not static, we can have problems.
...
```sh
appjail quick jtest overwrite=force start osarch=i386 osversion=13.2-RELEASE
```
...
```sh
appjail quick jtest overwrite=force start osarch=arm osversion=13.2-RELEASE release=armv7
```
...
```sh
appjail quick jtest overwrite=force start osarch=amd64 osversion=13.2-RELEASE type=thick
```
...
**Note**: As you can guess, the empty release follows the same approach as jails that come from a source tree, since they have a dummy file indicating that they are an empty release, jails that use them depend on said release directory and its files.
